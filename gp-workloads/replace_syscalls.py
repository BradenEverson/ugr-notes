...
"""
Replace occurrences of "syscall_<number>" with the correct syscall name.

Examples:
  Dry run (show diffs only):
    python replace_syscalls.py path/to/file.c

  In-place replace with backup files:
    python replace_syscalls.py src/ --in-place --backup

  Replace with "syscall_<name>" (preserve prefix):
    python replace_syscalls.py src/ --in-place --with-prefix

Notes:
- Unknown syscall numbers are left unchanged and reported.
- Uses a Linux x86_64-style mapping based on your provided list (0..334 and 424..451).
"""
...
# ----------------------------
# Syscall mapping (from your table)
# ----------------------------
SYSCALLS: Dict[int, str] = {
    ...
    17: "pread", 18: "pwrite", 19: "readv", 20: "writev", 21: "access", 22: "pipe",
    ...
    64: "semget", 65: "semop", 66: "semctl", 67: "shmdt", 68: "msgget",
    69: "msgsnd", 70: "msgrcv", 71: "msgctl", 72: "fcntl", 73: "flock",
...
PATTERN = re.compile(r"\bsyscall_(\d+)\b")

def replace_in_text(s: str, with_prefix: bool=False) -> Tuple[str, List[int]]:
    """
    Replace all occurrences of 'syscall_<num>' in the given string.
    Returns (new_text, unknown_numbers_list)
    """
    unknown: List[int] = []

    def _repl(m: re.Match) -> str:
        num = int(m.group(1))
        name = SYSCALLS.get(num)
        if name is None:
            unknown.append(num)
            return m.group(0)  # leave unchanged
        return f"syscall_{name}" if with_prefix else name

    new_s = PATTERN.sub(_repl, s)
    return new_s, sorted(set(unknown))

def iter_files(paths: List[str], exts: List[str], recursive: bool) -> List[str]:
    files: List[str] = []
    want_all = not exts
    normalized_exts = {e.lower() if e.startswith(".") else "." + e.lower() for e in exts}
    for path in paths:
            ...
            if recursive:
                for root, _, fnames in os.walk(path):
                    for fn in fnames:
                        ...
                        if want_all or os.path.splitext(fn)[1].lower() in normalized_exts:
                            files.append(full)
            else:
...
def process_file(path: str, in_place: bool, backup: bool, with_prefix: bool, dry_run: bool) -> int:
    try:
        with open(path, "r", encoding="utf-8") as f:
            original = f.read()
    except UnicodeDecodeError:
        # fall back to latin-1 to avoid choking on mixed encodings
        with open(path, "r", encoding="latin-1") as f:
            original = f.read()

    new_text, unknown = replace_in_text(original, with_prefix=with_prefix)

    if unknown:
        print(f"[WARN] {path}: unknown syscall numbers encountered: {sorted(set(unknown))}", file=sys.stderr)

    if new_text == original:
        print(f"[SKIP] {path}: no changes.")
        return 0

    if dry_run and not in_place:
        print(f"--- {path} (preview) ---")
        # Simple preview: show changed lines
        for old_line, new_line in zip(original.splitlines(), new_text.splitlines()):
    ...
    # Default: print to stdout if not in-place and not dry-run
    sys.stdout.write(new_text)
    return 0

def main():
    ap = argparse.ArgumentParser(description="Replace 'syscall_<number>' with the named syscall.")
    ap.add_argument("paths", nargs="+", help="Files and/or directories to process")
    ap.add_argument("--ext", action="append", default=[], help="File extensions to include (e.g., --ext .c --ext .h). Default: all")
    ap.add_argument("--recursive", action="store_true", help="Recurse into directories")
    ap.add_argument("--in-place", action="store_true", help="Edit files in place")
    ap.add_argument("--backup", action="store_true", help="Write a .bak backup when using --in-place")
    ap.add_argument("--with-prefix", action="store_true", help='Replace with "syscall_<name>" instead of "<name>"')
    ap.add_argument("--dry-run", action="store_true", help="Preview changes without writing")
    args = ap.parse_args()

    files = iter_files(args.paths, args.ext, args.recursive)
    if not files:
